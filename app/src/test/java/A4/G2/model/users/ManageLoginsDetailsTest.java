...
public class ManageLoginsDetailsTest {
    ...
    @BeforeEach
    public void setup() {
        user1 = Mockito.spy(new User("jeff", "Qwerty30", "jeff.com", "123456",
                "jeff house", DateGenerator.getSampleDateOver16()));

        user2 = Mockito.spy(new User("Steve", "Qwerty30", "steve.com", "876543",
                "Steve house", DateGenerator.getSampleDateOver16()));

        userList.add(user1);
        userList.add(user2);

        Gallery gallery = Mockito.mock(Gallery.class);
        Mockito.when(gallery.getUserList()).thenReturn(userList);
        loginDetailsManager = new LoginDetailsManager(gallery);
    }

    @Test
    public void testUserChangesUsernameToUniqueUsername() {
        try {
            assertTrue(loginDetailsManager.changeUsername(user1, "Geoff"));
        } catch (Exception exception) {
            fail("This should have not thrown an exception");
        }
        Mockito.verify(user1, times(1)).setUsername("Geoff");
    }

    @Test
    public void testUserChangesUsernameToTakenUsername() {
        try {
            loginDetailsManager.changeUsername(user1, "Steve");
            fail("This should have thrown an exception");
        } catch (UsernameTakenException e) {
            Mockito.verify(user1, times(0)).setUsername("Steve");
    ...
    public void testUserChangesPasswordToWeakPassword_noNumbers() {
        try {
            loginDetailsManager.changePassword(user1, "Qwerty30", "Password", "Password");
            fail("This should have thrown an exception");
        } catch (WeakPasswordException | IncorrectPasswordException e) {
            Mockito.verify(user1, times(0)).setPassword("Password");
    ...
    public void testUserChangesPasswordToWeakPassword_noCapitalLetters() {
        try {
            loginDetailsManager.changePassword(user1, "Qwerty30", "password9", "password9");
            fail("This should have thrown an exception");
        } catch (WeakPasswordException | IncorrectPasswordException e) {
            Mockito.verify(user1, times(0)).setPassword("password9");
    ...
    public void testUserChangesPasswordToWeakPassword_lessThan6Chars() {
        try {
            loginDetailsManager.changePassword(user1, "Qwerty30", "Hi5", "Hi5");
            fail("This should have thrown an exception");
        } catch (WeakPasswordException | IncorrectPasswordException e) {
            Mockito.verify(user1, times(0)).setPassword("Hi5");
    ...
    public void testUserChangesPassword_oldPasswordIncorrect() {
        try {
            ...
            Mockito.verify(user1, times(1)).getPassword();
            Mockito.verify(user1, times(0)).setPassword("Password9");
    ...
    public void testUserIncorrectlyRetypesNewPassword() {
        try {
            loginDetailsManager.changePassword(user1, "Qwerty30", "Password9", "Qassword9");
            fail("This should have thrown an exception");
        } catch (IncorrectPasswordException | WeakPasswordException e) {
            Mockito.verify(user1, times(0)).setPassword("Password9");
    ...
    public void testUserChangesPasswordToStrongPasswordSuccessfully() {
        try {
            boolean result = loginDetailsManager.changePassword(user1, "Qwerty30", "Password9", "Password9");

            assertTrue(result);
            Mockito.verify(user1, times(1)).getPassword();
            Mockito.verify(user1, times(1)).setPassword("Password9");
